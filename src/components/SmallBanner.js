...
import { img1, img2,img5, img6 } from "../assets/images/images";
...
const SmallBanner = () => {
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            nextSlide();
        }, 3000); // set the interval to 5 minutes (300000 milliseconds)
    ...
    return (
        <div className='max-w-[1600px] h-[250px] w-full m-auto relative group bg-transparent'>

            <img className="object-cover object-center absolute w-full h-full -z-1" src={slides[currentIndex].url} alt="Hero_Section_Image" />
            <div style={{ backgroundImage: 'linear-gradient(rgba(0,0,0, 0.52), rgba(0,0,0, 0.73))' }} className='relative w-full h-full bg-transparent transition-all duration-500 ease-in-out text-Green m-auto  z-10'>
                <div className="py-6 px-4 mx-auto max-w-screen-xl text-center lg:py-12 lg:px-12">

                    <h1 className="mb-4 max-[530px]:text-2xl text-4xl font-extrabold tracking-tight leading-none text-white md:text-3xl lg:text-5xl">Empowering Transportation and Construction Solutions</h1>
            ...
            <div className='hidden group-hover:block absolute top-[50%] -translate-x-0 translate-y-[-50%] left-5 text-2xl rounded-full p-2 bg-black/20 text-white cursor-pointer z-20'>
                ...
                < FontAwesomeIcon icon={faChevronCircleLeft} onClick={prevSlide} size={30} />
            </div>
            {/* Right Arrow */}
            <div className='hidden group-hover:block absolute top-[50%] -translate-x-0 translate-y-[-50%] right-5 text-2xl rounded-full p-2 bg-black/20 text-white cursor-pointer z-20'>
                ...
                <FontAwesomeIcon icon={faChevronCircleRight} onClick={nextSlide} size={30} />
            </div>
        </div>
    );
}
